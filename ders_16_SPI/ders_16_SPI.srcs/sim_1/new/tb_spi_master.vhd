...
-- Component Declaration for the Unit Under Test (UUT)
	...
	signal SPISIGNAL 	: std_logic_vector(7 downto 0) := (others => '0');  -- Göndermek istediğimiz test sinyalini buna ayzıcaz
	signal spiWrite 	: std_logic := '0';
	signal spiWriteDone : std_logic := '0';   
 
begin
 
-- Instantiate the Unit Under Test (UUT) (Test Edilen Birimi Örneklendirin)
	uut: spi_master 
	...
	SPIWRITE_P : process begin
		...
		wait until rising_edge(spiWrite);	-- spiWrite ne zaman 1 olursa. Aşağıda bunu 1 yapıcaz 
		
		-- Bu iki durum için geçerli
		-- for cpol = 1 cpha = 1
		-- for cpol = 0 cpha = 0
	 
		miso_i <= SPISIGNAL(7);
		wait until falling_edge(sclk_o); -- Alçalan kenara kadar bekle
		miso_i <= SPISIGNAL(6);
