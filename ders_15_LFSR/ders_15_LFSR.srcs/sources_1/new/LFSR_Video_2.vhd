...
entity LFSR_Video_2 is
	
	generic (
		c_clkfreq   : integer := 100_000_000;
		c_datawidth : integer := 10;			-- Kaç bitlik bir lfsr olacağı generic olsun istediğimiz zaman değiştirelim.
		c_shiftfreq : integer := 100			-- Her bir hesaplama adımı ne kadar zamanda yapılacak. Önceki tasarımda her clocta bir işlem yapıyordu
		-- fakat şimdi her clockta bir işlem yaparsak hızlı bir şekilde işlerini bitirmiş olacak yani 10 mikro saniyede bir tur atacak
		-- Onun yerine bir shift frekans koyduk. Böylelikle 10 milisaniye de bir bir shift işlemi yapacak. Bu şekilde 1024 taneyi 10 saniyede yapar.
		-- Böyle yapılmasının sebebi kura çekerken durdurma zamanımız resetleme zamanımız flan olsun diye
	);
...
architecture Behavioral of LFSR_Video_2 is

	constant c_timer_lim : integer := c_clkfreq / c_shiftfreq;   -- 10 milisaniye
	...
	signal timer 	 : iteger range 0 to timer_lim := 0;
...
begin
	...
	begin
		...
		if (rising_edge(clk)) then
			...
			if (enable_i = '1') then	-- Enable olunca sayacak
			
				if (timer = c_timer_lim)   then
